...
Complete the following steps to configure an Azure Event Hubs data selection. Use the `PUT` method in conjunction with the `api/v1/configuration/<ComponentId>/DataSelection/<StreamId>` REST endpoint to initialize the configuration.
...
6. Enter the following cURL command (which uses the `PUT` method) to initialize the client settings configuration.
    ...
    ```bash
    curl -d "@ConfigureClientSettings.json" -H "Content-Type: application/json" -X PUT "http://localhost:5590/api/v1/configuration/AzureEventHubs1/DataSelection/SampleStreamId"
    ```

    **Notes:**
  
    * If you installed the adapter to listen on a non-default port, update `5590` to the port number in use.
    * If you use a component ID other than `AzureEventHubs1` and a stream ID other than `SampleStreamId`, update the endpoint with your chosen component ID and stream ID.
