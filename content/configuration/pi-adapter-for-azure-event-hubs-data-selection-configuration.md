...
| **DataFields** | <sup>1</sup> | `string` | A `ComplexTypeMapping` that maps JSONPath expressions of fields to property names. Supported complex data types are `Coordinates` (x, y, and z) or `Geolocation` (latitude or longitude).<br><br>[Complex data type field mapping examples](#complex-data-type-field-mapping-examples)
